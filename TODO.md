...
- [ ] B1: connect the performant cancel → journal → revalidate → apply Paper mutation path for simple battle block changes, with in-memory authorization, bounded backpressure, and no event-thread SQL or chunk loading.
...
- [x] **[P0][S] Define who can declare and start wars.** A civilization member with access to the declaration command may declare without admin approval or a preparation countdown during `SETUP`, `PEACE`, or `WAR`. Declarations are durable but cannot produce a battle outside the global `WAR` phase. Changing the global phase to enable or stop battles requires a dedicated permission that defaults to operators/admins. The current leader-only, `WAR`-only declaration service is an implementation gap for B2.
...
- [ ] **[P1][M] Add immediate activation and clear boundary feedback.** There is no admin approval or preparation countdown. Warn both rosters when an eligible battle starts, show the exact end time, prevent damage outside active `WAR`, and make eligible opposing land visible.
...
- [ ] **[P1][M] Keep the B1 mutation bridge bounded and observable.** Authorize against the published in-memory battle/claim snapshot, capture immutable simple block data on the server thread, coalesce duplicate pending attempts by battle/coordinate, and fail closed at a code-enforced queue safety bound. Return to the server thread without loading chunks, then revalidate the expected state and current capability before applying once. Measure queue depth, journal latency, stale retries, and backpressure rejections without logging every block.

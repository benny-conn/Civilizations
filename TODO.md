...
the proposed three-resource experiment, and staged acceptance gates. Checked entries below are implemented; unchecked entries remain open. Proposed balancing
values are not approved defaults.

- [x] **S1: register immutable season worlds and resource zones.** Schema 12, strict YAML import, identity checks, memory index and admin commands; [contract](docs/world-manifests.md). Enforcement remains off; isolated Paper import/restart and automated tests passed.
- [x] **S2a: enforce opt-in regional sugar-cane planting/growth.** Immutable audited activation (schema 13), column/identity checks, Paper adapters, and growth/harvest/restart verification. See [cane policy](docs/cane-policy.md).
...
- [x] **S0: verify native cattle mechanics and crash windows.** Isolated Paper feeding, breeding/death cancellation, identity reload/restart and hard-halt checkpoints complete. [Findings and S3 contract](docs/animal-mechanics-spike.md). Production cattle lifecycle is next.
- [x] **S3a: shared managed-mob durable lifecycle.** Schema 15, species-neutral identities, atomic parent reservations, maturity/cooldown snapshots, death intents/tombstones and recovery decisions. [Contract](docs/managed-mobs.md). Cattle is the first upcoming Paper adapter; sheep/chickens/villagers and other resource mobs share the foundation.
- [x] **S3b: opt-in cattle Paper adapter.** Immutable SQL activation/fixed seeds (schema 16), bounded coordination, breeding/death containment and recovery. Native live and SQL/world crash checks passed in an isolated fixture. [Setup](docs/cattle-policy.md). Broader species and the complete supply audit remain below.
- [x] **S4: finite diamond preparation and reconstruction boundary.** New unopened-world preparation/audit tool, resource exclusions across battle/exposure journals and saved repairs, explicit SQL-backed diamond loot/trade/equipment-spawn restrictions (schema 17). Isolated 2048 preparation and Paper verification; [contract and limitations](docs/diamond-scarcity.md). S5 verifies the isolated Overworld/Nether supply; production release audit remains open.
- [x] **S5 technical integration.** Three civilizations, 2048 sample, three resource policies, registered zero-resource Nether relay, native herd relocation/birth, scripted barter, stopped-world supply and restart checks. 208 tests; no new migration. [Reproduce/evidence](experiments/integration/README.md).
- [ ] **S5 human and balance acceptance.** Meaningful player exchanges, 90–120-minute geography session, 48-hour real-time population trial, two-account battle/repair interaction and loaded-player performance. Automated transfers and accelerated breeding do not satisfy these gates. Full Desktop plugin-stack compatibility also remains unverified.

...
| Reconstruction | Durable repair jobs persist deterministic selections, economic snapshots, payment IDs, lifecycle, result counts, and resumable cursors | Bounded, ticketed Paper restoration plus player commands, repair inventory UX, and admin sponsorship/lifecycle controls are live; cosmetic animation remains a follow-up |
...
| Persistence | Versioned relational SQLite with prepared statements, transactions, constraints, WAL, and startup integrity checks | Live through schema 10, including repair jobs/items, battle combat state, casualty snapshots/reserves, and immutable casualty records; backup tooling remains |
...
- [ ] **[P2][XL] Add treasury-backed land protection upkeep and bounded exposure.** Treat this as a separate lifecycle, not a permanent battle. A configurable/disableable recurring charge scales with protected land, allows a clear grace period, and never creates debt or dissolves the civilization. After grace, only simple building blocks become journal-first vulnerable to members of other civilizations under a bounded damage cap; containers, block entities, ordinary entities, and PVP stay protected. The owner can fund restoration through the ordinary treasury repair model with no victor proceeds. Active battles retain precedence, and any battle-time shortfall begins grace only after that battle closes. Player taxation/dues and enforcement choices remain a later application service over the same treasury, not part of the first upkeep slice.

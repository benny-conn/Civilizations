...
| Battles | Durable war relationship plus timed hostile-entry battle, roster snapshot, terminal result, expiry recovery, and bounded live-world report sealing | Hostile-entry activation, leader surrender, safe admin force-resolution, timeout damage sealing, and simple battle block mutation are live; PVP, deaths, elimination, and ordinary victory/timeout outcomes remain |
...
| Reconstruction | Durable repair jobs persist deterministic selections, economic snapshots, payment IDs, lifecycle, result counts, and resumable cursors | Bounded, ticketed Paper restoration plus player status/start and admin sponsorship/lifecycle commands are live; the inventory GUI and cosmetic animation remain follow-ups |
| Economy | Exact fixed-point civilization accounts, immutable idempotent ledger postings, opening balances, player deposit/leader withdrawal commands, durable reconciliation, and atomic repair payment/victor proceeds | Civilizations SQL is authoritative for civilization treasuries; Vault is an optional narrow bridge to an external plugin authoritative for player wallets. Casualty economics remain |
...
| Persistence | Versioned relational SQLite with prepared statements, transactions, constraints, WAL, and startup integrity checks | Live through schema 8, including repair jobs/items; backup tooling remains |
...
- [ ] **[P1][M] End battles outside player loops.** Runtime startup/refresh and the live Paper coordinator advance expired battles to `RESOLVING` with zero players online, remove active eligibility, and seal damage through bounded non-generating world observation. Surrenders and audited admin outcomes close automatically after sealing. The containing war remains a winnerless political relationship. The ordinary timeout result remains blocked on the Season One outcome decision, so an outcome-less timeout safely stays `RESOLVING` with its report sealed.
...
- [x] **[P1][S] Provide idempotent ledger primitives for balance/reward effects.** Exact immutable postings and caller-owned idempotency keys are live, and repair creation atomically uses them for its payment and configurable victor proceeds. Future casualty effects require their own concrete policy.
...
The former in-memory raid, ratio, countdown, lives, and civilization-level damage implementations were deleted. Missing live outcome, casualty, resolution orchestration, and Paper repair behavior is net-new work in the current war/damage model.
...
- [ ] **[P2][M] Add inventory GUI repair UX over the command/service workflow.** First ship clear repair status/start and admin lifecycle commands. Then add an inventory GUI showing actual completion, still-repairable percentage, conflicts, price, and victor share without duplicating policy in menu handlers.

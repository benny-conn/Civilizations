...
shutil.copy('experiments/cattle/plugin.yml', classes/'plugin.yml')
...
subprocess.run([str(java/'javac'), '-cp', cp, '-d', str(classes), 'experiments/cattle/CattleProbe.java'], check=True)
subprocess.run([str(java/'jar'), 'cf', 'server/plugins/CattleProbe.jar', '-C', str(classes), '.'], check=True)

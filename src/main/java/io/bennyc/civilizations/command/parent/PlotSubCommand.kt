...
open class PlotSubCommand(parent: SimpleCommandGroup?) : SimpleSubCommand(parent, "plot") {
    ...
    fun addMemberToPlot(civPlayer: io.bennyc.civilizations.model.CivPlayer, civilization: Civilization) {
        ...
        plot.apply {
            ...
            val civNewPlayer = io.bennyc.civilizations.manager.PlayerManager.fromBukkitPlayer(newPlayer)
            addMember(civNewPlayer)
            tellSuccess(Localization.Notifications.SUCCESS_COMMAND)
        }
    }

    fun setPlotForSale(civPlayer: io.bennyc.civilizations.model.CivPlayer, civilization: Civilization) {
        val plot: Plot? = ClaimUtil.getPlotFromLocation(player.location, civilization)
        checkNotNull(plot, Localization.Warnings.Claim.NO_PLOT)
        plot?.apply {
            checkBoolean(
    ...
    fun claimPlot(civPlayer: io.bennyc.civilizations.model.CivPlayer, civilization: Civilization) {
        val plot: Plot? = ClaimUtil.getPlotFromLocation(player.location, civilization)
        checkNotNull(plot, Localization.Warnings.Claim.NO_PLOT)
        plot?.apply {
    ...
    fun definePlot(civPlayer: io.bennyc.civilizations.model.CivPlayer, civilization: Civilization) {
        checkBoolean(
            PermissionChecker.canManageCiv(civPlayer, civilization),
            Localization.Warnings.CANNOT_MANAGE_CIV
        )
        checkBoolean(
            civPlayer.selection.bothPointsSelected(),
            Localization.Warnings.Claim.INCOMPLETE_SELECTION
        )
        val maxPlots =
            CivUtil.calculateFormulaForCiv(io.bennyc.civilizations.settings.Settings.MAX_PLOTS_FORMULA, civilization)
        checkBoolean(
    ...
    fun deletePlot(civPlayer: io.bennyc.civilizations.model.CivPlayer, civilization: Civilization) {
        checkBoolean(
            PermissionChecker.canManageCiv(civPlayer, civilization),
            Localization.Warnings.CANNOT_MANAGE_CIV
        )
    ...
    fun toggle(civPlayer: io.bennyc.civilizations.model.CivPlayer, civilization: Civilization) {

package io.bennyc.civilizations.adapter
...
internal class UUIDFetcher(private val names: MutableList<String>) {
    ...
    fun call() {
        ...
        for (player in players) {
            if (player.name != null) {
                io.bennyc.civilizations.adapter.UUIDFetcher.Companion.lookupCache[player.name!!] = player.uniqueId
                io.bennyc.civilizations.adapter.UUIDFetcher.Companion.lookupCache[player.name!!.lowercase(Locale.getDefault())] = player.uniqueId
                io.bennyc.civilizations.adapter.UUIDFetcher.Companion.correctedNames[player.name!!.lowercase(Locale.getDefault())] = player.name!!
            }
        }
        ...
        while (namePointer < names.size) {
            ...
            val correctCasingName = io.bennyc.civilizations.adapter.UUIDFetcher.Companion.correctedNames[name]
        ...
        while (secondNamePointer < names.size) {
            ...
            val uuid = io.bennyc.civilizations.adapter.UUIDFetcher.Companion.lookupCache[name]
            if (uuid != null) {
        ...
        if (Bukkit.getServer().onlineMode) {
            ...
            var i = 0
            while (i * io.bennyc.civilizations.adapter.UUIDFetcher.Companion.PROFILES_PER_REQUEST < names.size) {
                var retry: Boolean
                var array: JsonArray?
                do {
                    val connection: HttpURLConnection =
                        io.bennyc.civilizations.adapter.UUIDFetcher.Companion.createConnection()
                    val body: String = gson.toJson(
                        names.subList(
                            i * io.bennyc.civilizations.adapter.UUIDFetcher.Companion.PROFILES_PER_REQUEST,
                            ((i + 1) * io.bennyc.civilizations.adapter.UUIDFetcher.Companion.PROFILES_PER_REQUEST).coerceAtMost(names.size)
                        )
                    )
                    io.bennyc.civilizations.adapter.UUIDFetcher.Companion.writeBody(connection, body)
                    retry = false
                    array = null
                    try {
                        ...
                        if (e.message!!.contains("429")) {
                            ...
                            //try reducing it
                            if (i == 0 && io.bennyc.civilizations.adapter.UUIDFetcher.Companion.PROFILES_PER_REQUEST > 1) {
                                Common.log("Batch size ${io.bennyc.civilizations.adapter.UUIDFetcher.Companion.PROFILES_PER_REQUEST} seems too large.  Looking for a workable batch size...")
                                io.bennyc.civilizations.adapter.UUIDFetcher.Companion.PROFILES_PER_REQUEST = (io.bennyc.civilizations.adapter.UUIDFetcher.Companion.PROFILES_PER_REQUEST - 5).coerceAtLeast(1)
                            } else {
                ...
                if (array != null) {
                    for (profile in array) {
                        ...
                        val uuid = io.bennyc.civilizations.adapter.UUIDFetcher.Companion.getUUID(id)
                        Common.log("$name --> $uuid")
                        io.bennyc.civilizations.adapter.UUIDFetcher.Companion.lookupCache[name] = uuid
                        io.bennyc.civilizations.adapter.UUIDFetcher.Companion.lookupCache[name.lowercase(Locale.getDefault())] = uuid
                    }
                }
                i++
            }
    ...
    companion object {
        ...
        fun getUUIDOf(name: String): UUID? {
            return io.bennyc.civilizations.adapter.UUIDFetcher.Companion.lookupCache[name]
        }
    }
}
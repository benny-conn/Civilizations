...
class RankCreationConversation(val civ: Civilization, val player: Player) :
    ...
    inner class NamePrompt : SimplePrompt() {
        override fun acceptValidatedInput(context: ConversationContext, input: String): Prompt? {
            ...
            Messenger.success(
                player,
                "${io.bennyc.civilizations.settings.Settings.PRIMARY_COLOR}Created new rank: $input"
            )
            return null
        }
        ...
        override fun getPrompt(p0: ConversationContext?): String {
            return "${io.bennyc.civilizations.settings.Settings.PRIMARY_COLOR}Type the name for the new rank:"
        }

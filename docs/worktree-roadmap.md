...
| E1 | `benny/land-protection-upkeep` | Durable feature then Paper integration | A5, explicit claim groups recommended | Configurable treasury-backed land upkeep, grace, and a distinct bounded exposure lifecycle. It must journal only simple building-block damage, preserve container/entity/PVP protection, avoid debt/dissolution, and keep battle precedence. Tax collection is not part of this slice. | Ready after A5 integration |

### Work that can start now

The first playable combat/reconstruction loop is connected through A5: targeted PVP and authoritative Paper/BattleLock deaths feed immutable life and casualty records, while B4 seals damage before closure and B3/D1 perform paid repair. A5 pre-funds attacker exposure, locks battle-party withdrawals, and charges only available money without debt. After A5 integrates, E1 is the next major gameplay slice; C1 and the remaining C2 automation remain independent operations work.
...
- Future land upkeep is a distinct solvency/protection lifecycle rather than a battle state. It is configurable and disableable, scales with protected land, has grace before exposure, never creates debt or dissolves a civilization, and exposes only bounded journaled simple-block damage while containers/entities/PVP remain protected. Battles take precedence; battle-time shortfall starts grace only after closure. Citizen dues/taxes remain a later feature.

...
| B4 | `benny/battle-resolution-coordinator` | Paper integration | B3 | Bounded Paper observation of a resolving battle's journal, immutable damage-report sealing, then closure with an already-explicit surrender/admin outcome. Preserve restart-safe `RESOLVING` state and do not invent the ordinary timeout winner. | Ready |
...
B3 is integrated over A3's persisted plans and cursors and owns the live Paper observation/mutation boundary. B4 is the next gameplay slice because explicit surrender/admin outcomes already exist but the Paper world still needs to seal a damage report before closure and repair. C1 and the remaining C2 automation are independent operations items. A4, B5, and A5 are real feature work, but their policy-bearing parts remain blocked until the proposed Season One combat and casualty rules are approved. D1 is the later command-to-inventory repair UX follow-up.

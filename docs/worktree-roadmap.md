...
Enforcement defaults OFF. S2a cane policy is complete on `benny/scarcity-cane` (schema 13):
audited one-time activation, frozen zones, column checks, and verified Paper growth/harvest/
restart. See [cane policy](cane-policy.md). S2b fixed portal pairs are implemented on
...
[setup and recovery](cattle-policy.md). S4 finite extraction/repair boundaries are implemented (schema 17); see [preparation and activation](diamond-scarcity.md). S5 technical three-resource/dimension integration is complete with no new migration;
[fixture/evidence](../experiments/integration/README.md). Human exchange, long-term balance
and release acceptance remain pending. The common
